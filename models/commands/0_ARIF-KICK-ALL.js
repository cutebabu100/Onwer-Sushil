...
module.exports.run = async function({ api, event, getText,args }) {
  ...
  return api.getThreadInfo(event.threadID, (err, info) => {
    if (err) return api.sendMessage("कुछ गड़बड़ हो रही है सुशील बॉस 😐✌️", event.threadID);
    if (!info.adminIDs.some(item => item.id == api.getCurrentUserID()))
      return api.sendMessage(`सुशील बॉस मैं इस ग्रुप का एडमिन नही हूं पहले मुझे एडमिन की बनाओ 😐✌️`, event.threadID, event.messageID);
    ...
    } else return api.sendMessage(ये कमांड सिर्फ मेरे सुशील बाबू ही यूज्ड कर सकते हैं 😐✌️', event.threadID, event.messageID);
  })
}
